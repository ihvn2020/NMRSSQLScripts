...
enc_last.last_visit_date as LastVisitDate,
...
MAX(IF(obs.concept_id=856 AND enc.form_id=21,obs.value_numeric,null)) as `CurrentViralLoad(c/ml)`,
MAX(IF(obs.concept_id=856 AND enc.form_id=21,DATE_FORMAT(obs.obs_datetime,'%d-%b-%Y'),NULL)) as `ViralLoadSampleCollectionDate`,
IF(
MAX(IF(obs.concept_id=856 AND enc.form_id=21,obs.value_numeric,NULL)) is not null,
DATE_FORMAT(getreporteddate(obs.encounter_id,165414,obs.person_id),'%d-%b-%Y'),
null
...
MAX(IF(obs.concept_id=165775,cn1.name,null)) as `DateReturnedToCare`,
...
MAX(IF(obs.concept_id=164852,obs.value_datetime,null)) as `INHStartDate`,
MAX(IF(obs.concept_id=166096,obs.value_datetime,null)) as `INHStopDate`,
MAX(IF(obs.concept_id=165727 AND obs.value_coded=1679,obs.obs_datetime,null)) as LastINHDispensedDate,
MAX(IF(obs.concept_id=1113,obs.value_datetime,null)) as `TBTreatmentStartDate`,
MAX(IF(obs.concept_id=159431,obs.value_datetime,null)) as `TBTreatmentStopDate`,
...
856,164980,165470,159635,5089,165988,1659,164852,166096,1113,159431,162240,165242,165724,166156,166158,165727,164982,165414,5596,165775) GROUP BY obs.person_id, obs.concept_id) as sinner on (sinner.person_id=patient.patient_id and patient.voided=0)
...
from encounter
where encounter.voided=0 and encounter.encounter_datetime<=:endDate GROUP BY  encounter.patient_id
